...
> **状态：历史设计方案，部分结论已被实现替代（2026-08-25）**
>
> 本文保留当初的分析与推演过程，供追溯设计取舍。以下几处**已不代表当前实现**，
> 阅读时请以《UI定制.md》为准：
>
> - 文中「按 `SANYI_CLIENT_ID` 编译期选客户、每客户一个构建产物、CMake 生成
>   per-client `.qrc`」的方案**未采用**。实际实现是**客户 ID 运行时解析**
>   （`UiClientContext`：override > 环境变量 > `QSettings` > `san_yi`），
>   所有客户 JSON 一起打进 qrc，**一份二进制服务全部客户**。
> - 文中的过渡开关 `SANYI_ENABLE_CONFIG_DRIVEN_UI` 已废弃，配置驱动是唯一路径。
> - 客户专属 C++ 不做条件编译，改为运行期按 `clientId()` 注册。
>
> 当前实现的权威文档：`Docs/01-当前架构/UI定制.md`。
