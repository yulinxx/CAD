...
3D 走实体自带的 `SyMeshEntity::locked()`，各自打同形的 WARN 日志。
...
before == after，压进去只会得到一条"撤销后画面毫无变化"的空记录，还白占 200 条的栈上限。
...
选中表必须已经收敛，且实体本身仍然可解引用。**

- `removeEntity()`：`release` 出裸指针 → `erase` + 空间索引 + 选中表 + `++generation`
  → 最后 `notifyEntityRemoved` / `notifySceneChanged`。实体所有权已交调用方，通知期间还活着。
- `deleteEntities()`：先把待删的 `unique_ptr` 整体摘进局部 `extracted`（容器同步 erase）
  → `++generation` → 通知 → 函数返回时 `extracted` 析构，实体在那时才真正释放。
...
- `deleteEntities` 曾把整批通知全发完才开始 erase：观察者
  （`SelectionManager3D::syncSelectionFromScene`）回读场景时实体还在，判定"仍然存活"
  把它们留在选中表里，紧接着实体就被销毁 → 选中表短暂持有已释放指针。
