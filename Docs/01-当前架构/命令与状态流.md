...
`View_Reset` 六条命令在 `Main/Src/Composition/CoreOperationRegistry.cpp` 里注册成
...
新增视图类命令时按同一形状办：在 `CoreOperationRegistry` 里注册 → 交给
...
| handler | `CoreOperationRegistry::registerAll` | `EditOperations3D::registerAll` |
...
- 那三处的注释都写着「旧 OperationBus 的 OperationContext 在生产路径不绑定场景，走总线只会被拒绝」——
  这个前提早已失效：`CoreOperationRegistry` 注册的 `LambdaOperation` 直接捕获 `editService`，
