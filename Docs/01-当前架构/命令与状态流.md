...
- 2026-08-27 删除链路收敛：2D 的三条额外 Delete 通路（`SelectTool::handleDeleteKey` / `ViewportInputRouter::handleDeleteKeyPress` / `RenderViewport2D::deleteSelectedEntity`）删除，窗口级快捷键改为 `Qt::ApplicationShortcut` + `OperationBus::run(Edit_Delete, Shortcut)`；3D 执行层补上与 2D 同语义的锁定拒绝；`SceneManager3D` 两个移除 API 的通知时序统一（见 4.4 与 9.4）
