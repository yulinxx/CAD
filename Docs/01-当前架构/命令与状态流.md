# 命令与状态流

## 概述

本文档描述命令的生命周期、状态管理机制、以及相关的数据流和事件流。

---

## 1. 命令生命周期

### 1.1 完整流程

```
用户操作 (按钮/菜单/快捷键/Gizmo)
    │
    ▼
OperationBus::run(OperationId, params, source) / 命令统一入口
    │
    ├─→ 检查操作是否注册
    │
    ├─→ 构建 DispatchRequest
    │
    ├─→ dispatchOperation() 执行
    │       │
    │       ├─→ 检查前置条件 (canExecute)
    │       │
    │       ├─→ 执行操作 (execute)
    │       │       │
    │       │       ├─→ 预览 (preview)
    │       │       │
    │       │       ├─→ 提交 (commit)
    │       │       │
    │       │       └─→ 取消 (cancel)
    │       │
    │       ├─→ 刷新视图 (refresh)
    │       │
    │       └─→ 记录日志
    │
    ├─→ onOperationCompleted() 回调
    │
    ├─→ postExecute() 后处理
    │
    └─→ 发出 operationCompleted 信号
```

### 1.2 命令注册

```cpp
// 在 ApplicationCompositionRoot 中注册操作
OperationBus* bus = compositionRoot->operationBus();
// 新代码统一通过 LambdaOperation / ParamLambdaOperation 捕获服务，不依赖 OperationContext
bus->registerOperation(std::make_unique<LambdaOperation>(OperationId::Edit_Delete, [editService] {
    editService->deleteSelected("Delete");
}));
// 所有命令统一注册到 OperationBus
```

### 1.3 命令执行

```cpp
// 用户触发操作
OperationResult result = OperationBus::instance()->run(
    OperationId::Tool_Line,
    QVariantMap(),
    OperationSource::Toolbar
);

// 操作实现
class LineToolOperation : public IOperation
{
    OperationResult execute(const OperationRequest& req) override
    {
        // 1. 获取上下文
        auto& ctx = getContext();
        
        // 2. 执行操作
        // ...
        
        // 3. 返回结果
        return { true, "Line created" };
    }
};
```

---

## 2. 状态管理

### 2.1 核心状态

| 状态 | 定义 | 管理位置 |
|------|------|----------|
| **selection** | 当前选中的实体集合 | `SelectionSet` / `SelectionManager` |
| **activeTool** | 当前活动工具 | `ToolManager` |
| **commandStage** | 当前命令阶段 | `ToolContext` / 命令上下文 |
| **isBusy** | 是否正在执行操作 | `OperationBus` |
| **isDirty** | 文档是否被修改 | `SceneDocument2D/3D` / `DocumentTransaction` |

### 2.2 状态流转

```
                     ┌──────────────┐
                     │   Idle       │
                     │  (无活动命令) │
                     └──────┬───────┘
                            │ 用户触发命令
                            ▼
                     ┌──────────────┐
                     │  Executing   │
                     │  (命令执行中) │
                     └──────┬───────┘
                            │
              ┌──────────────┼──────────────┐
              ▼              ▼              ▼
        ┌─────────┐   ┌─────────┐   ┌─────────┐
        │ Preview │   │  Commit │   │ Cancel  │
        │ (预览中) │   │ (已提交) │   │ (已取消) │
        └────┬────┘   └────┬────┘   └────┬────┘
             │             │             │
             └─────────────┼─────────────┘
                           ▼
                     ┌──────────────┐
                     │   Idle       │
                     └──────────────┘
```

### 2.3 Selection 状态

```cpp
// SelectionSet 接口
class SelectionSet
{
public:
    void add(const Uuid& id);
    void remove(const Uuid& id);
    void clear();
    bool contains(const Uuid& id) const;
    const std::unordered_set<Uuid>& getIds() const;
    
    // 信号
    void onSelectionChanged();
};
```

### 2.4 Undo / Redo 状态

```cpp
// QtUndoRedoBridge 接口
class QtUndoRedoBridge
{
public:
    void pushUndo(std::unique_ptr<QUndoCommand> cmd);
    void undo();
    void redo();
    bool canUndo() const;
    bool canRedo() const;
};
```

---

## 3. 工具状态机

### 3.1 工具生命周期

```
┌────────────┐      ┌────────────┐      ┌────────────┐
│  Inactive  │─────→│  Activated │─────→│   Active   │
│  (未激活)   │      │   (激活中)   │      │  (活动中)   │
└────────────┘      └────────────┘      └──────┬─────┘
       ▲                                        │
       │                                        │ 用户操作
       └────────────────────────────────────────┘
              (工具切换或取消)
```

### 3.2 工具状态转换

```cpp
// BaseTool 状态机
class BaseTool : public ITool
{
public:
    void onActivate() override;    // 工具激活
    void onDeactivate() override;  // 工具失活
    
    void onMousePress(QMouseEvent* event) override;
    void onMouseMove(QMouseEvent* event) override;
    void onMouseRelease(QMouseEvent* event) override;
    
protected:
    void startPreview();   // 开始预览
    void updatePreview();  // 更新预览
    void commitPreview();  // 提交预览
    void cancelPreview();  // 取消预览
};
```

---

## 4. 命令分类

### 4.1 按功能分类

| 分类 | 示例 | 是否可撤销 |
|------|------|-----------|
| **编辑操作** | Undo, Redo, Delete, Copy, Move | ✅ |
| **绘图工具** | Line, Circle, Arc, Rectangle | ✅ |
| **视图操作** | ZoomFit, ZoomIn, ResetView | ❌ |
| **文件操作** | New, Open, Save, Export | ✅ |
| **算法操作** | Nesting, Offset, Boolean | ✅ |
| **激光操作** | Connect, Home, StartProcess | ❌ |

### 4.2 按来源分类

| 来源 | 示例 | 文件 |
|------|------|------|
| **Menu** | 顶部菜单栏 | `MenuManager` |
| **Toolbar** | 工具栏按钮 | `LeftToolBar`, `TopToolBar` |
| **Shortcut** | 键盘快捷键 | `ShortcutManager` |
| **Gizmo** | 变换手柄 | `SelectionGizmo` |
| **ContextMenu** | 右键菜单 | `ViewWidget` |
| **Plugin** | 插件命令 | `PluginManager` |

---

## 5. 关键数据流

### 5.1 命令参数流

```
用户输入
    │
    ▼
QVariantMap params
    │
    ▼
OperationRequest
    │
    ├─→ id: OperationId
    ├─→ source: OperationSource
    ├─→ params: QVariantMap
    └─→ traceId: QString
    │
    ▼
DispatchRequest
    │
    ├─→ id: OperationId
    ├─→ source: OperationSourceKind
    ├─→ params: QVariantMap
    └─→ traceId: QString
```

### 5.2 命令结果流

```
OperationResult
    │
    ├─→ success: bool
    ├─→ message: QString
    ├─→ errorCode: int
    └─→ data: QVariantMap
    │
    ▼
onOperationCompleted(id, success, message)
    │
    ▼
sigOperationCompleted(id, success, message)
    │
    ▼
UI 更新 (状态栏、日志)
```

---

## 6. 状态同步机制

### 6.1 文档 → UI 同步

```
SceneDocument2D / DocumentTransaction
    │
    ├─→ sigSceneChanged()
    │       │
    │       ▼
    │   ViewRenderCoordinator::updateRenderData()
    │       │
    │       ▼
    │   RenderWidget::update()
    │
    ├─→ sigSelectionChanged()
    │       │
    │       ▼
    │   ViewWidget::syncSelectionFromScene()
    │
    └─→ sigDocumentModified()
            │
            ▼
        MainWindow::updateWindowTitle()
```

### 6.2 UI → 文档同步

```
用户操作
    │
    ▼
ToolManager::handleInput()
    │
    ▼
SceneEditServiceAdapter::execute()
    │
    ▼
SceneDocument2D::applyChange()
    │
    ▼
SceneDocument2D::notifyChange()
```

---

## 7. 边界检查清单

### 新增命令时检查
- [ ] 是否注册到 OperationBus？
- [ ] 是否实现 IOperation 接口？
- [ ] 是否正确处理 undo/redo？
- [ ] 是否正确记录日志？
- [ ] 是否返回正确的 OperationResult？

### 修改状态时检查
- [ ] 是否遵循状态机转换规则？
- [ ] 是否通知相关观察者？
- [ ] 是否处理取消/回滚？

---

---

## 8. 命令处理器拆分（2026-07-11 更新）

### 8.1 拆分文件结构

| 文件 | 职责 | 包含类/结构 |
|------|------|-------------|
| `CreateCommands.h/cpp` | 创建命令 | DrawLineCommand, CircleCommand, ArcCommand, PolylineCommand, PolygonCommand 及对应 UndoCommand |
| `TransformCommands.h/cpp` | 变换命令 | MoveCommand, RotateCommand, CopyCommand, DeleteCommand, MirrorCommand 及对应 UndoCommand |
| `SelectCommands.h/cpp` | 选择命令 | SelectCommand, SelectUndoCommand |
| `CommandSnapshots.h/cpp` | 快照辅助 | takeSnapshot, restoreFromSnapshot, restoreEntityGeometryFromSnapshot |
| `CommandGeometry.h/cpp` | 几何变换辅助 | rotatePoint, mirrorPoint, applyRotationToEntity |

### 8.2 EntitySnapshot 结构

```cpp
struct EntitySnapshot
{
    QString id;           // 图元 ID
    int type;             // 图元类型
    QPointF basePoint;    // 基点
    QVector<QPointF> points;  // 顶点列表
    double radius;        // 半径（圆/弧）
    double startAngle;    // 起始角度（弧）
    double endAngle;      // 终止角度（弧）
    bool closed;          // 是否闭合（多边形）
    bool ccw;             // 逆时针方向（多边形缠绕顺序）
};
```

### 8.3 IUndoStack 刷新回调机制

```cpp
class IUndoStack
{
public:
    using RefreshCallback = std::function<void()>;
    virtual void setRefreshCallback(RefreshCallback callback) = 0;
};

class DefaultUndoStack final : public IUndoStack
{
private:
    void notifyRefresh();  // 在 push/undo/redo 时调用
    
    RefreshCallback m_refreshCallback;
    bool m_isNotifying{ false };  // 防重入保护
};
```

**刷新流程**：命令提交 → `UndoStack::push()` → `notifyRefresh()` → 视图刷新回调 → 视口重绘

### 8.4 命令生命周期更新

> **2026-07-18 更新**：Undo 已统一由 `SceneEditService → UndoRedoManager` 管理。
> Ctrl+Z/Ctrl+Y 通过 `OperationBus::Edit_Undo/Edit_Redo` → `UndoRedoManager::undo/redo` 处理。
> 详见 `Docs/框架缺陷.md` P0 修复记录。
>
> **2026-07-22 更新**：`UiCommandDispatcher` 已移除，命令直接通过 `OperationBus` 调度，
> 交互式命令生命周期由 `IInteractionDispatcher` 管理（begin/submit/cancel）。

当前命令生命周期完整流程：

```text
用户操作 (按钮/菜单/快捷键)
    │
    ▼
OperationBus::run(opId) → 统一命令调度
    │
    └─→ Operation (LambdaOperation / ToolSwitchOperation / etc.)
            └─→ SceneEditService / IInteractionDispatcher (UndoRedoManager)

[交互式命令额外流程]
    │
    ├─→ IInteractionDispatcher::begin(commandId) → 标记命令开始
    │
    ├─→ [鼠标事件转发]
    │       ├─→ handler->onMouseDown → SceneEditService
    │       ├─→ handler->onMouseMove
    │       └─→ isComplete() → submit()
    │
    └─→ IInteractionDispatcher::submit()
            │
            ├─→ handler->commit()（图元已通过 SceneEditService 创建）
            ├─→ 清理状态中心
            └─→ handler->reset()
```

### 8.5 撤销/重做机制

> **2026-07-18 更新**：旧 `IUndoStack`/`ICommandHandler::createUndoCommand()` 不再使用。
> Undo/Redo 统一由 `UndoRedoManager` 管理（通过 `SceneEditService` 内部调用 `executeCommand()`）。

| 命令类型 | Undo 策略 | Redo 策略 |
|----------|-----------|-----------|
| 创建命令（Line/Circle/Arc/Polyline/Polygon） | `UndoRedoManager` → `AddEntitiesCommand::undo()` 移除图元 | `AddEntitiesCommand::redo()` 恢复图元 |
| 删除命令 | `UndoRedoManager` → `DeleteEntitiesCommand::undo()` 恢复图元 | `DeleteEntitiesCommand::redo()` 移除图元 |
| 变换命令（Move/Rotate/Mirror） | `UndoRedoManager` → `EntitySnapshotsCommand::undo()` 恢复快照 | `EntitySnapshotsCommand::redo()` 应用快照 |
| 复制命令 | `EntitySnapshotsCommand` 删除副本 + 恢复旧选区 | 重新复制并选中新图元 |

## 9. 变更记录

| 日期 | 版本 | 变更内容 | 作者 |
|------|------|----------|------|
| 2026-07-11 | 1.1.0 | 补充命令处理器拆分、EntitySnapshot、IUndoStack 刷新回调、撤销/重做机制 | 开发组 |
| 2026-07-10 | 1.0.0 | 初版：基于当前架构总览和代码库现状编写 | 架构组 |
