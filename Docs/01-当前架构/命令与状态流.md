...
>
> **2026-08-18 更新**：确立“所有图元修改必须经 SceneEditService”统一入口原则。
> 新增 `SceneEditService::groupEntities`/`ungroupSelectedEntities`/`addEntitiesAndGroup` 等接口，
> 使 组合/打散/文件导入/阵列/文字/二维码/图片粘贴 等操作全部纳入撤销栈。
> `UndoRedoManager` 新增历史裁剪、保存点、批处理、命令合并能力，与 2D/3D 对齐。
...
>
> **2026-08-18 更新**：撤销栈采用“双栈互换”算法（undoStack/redoStack），支持历史裁剪（200 条）、保存点、批处理、命令合并。
> 所有图元增删改、组合/打散、文件导入、阵列、文字/二维码/图片创建均经 `SceneEditService` 路由入栈。
> 新增 `GroupStateCommand` 处理嵌套群组撤销（文字、二维码等复杂层级）。
> 2D/3D 撤销核心算法统一：`undo()` 弹 undoStack → `undo()` → 压 redoStack；`redo()` 反之。
...
| 日期 | 版本 | 变更内容 | 作者 |
|------|------|----------|------|
| 2026-08-18 | 1.3.0 | 统一撤销架构：所有图元修改经 SceneEditService 单一入口；新增组合/打散/导入/阵列/文字/二维码/图片撤销；引入 GroupStateCommand、批处理、历史裁剪、保存点、命令合并；修复 ArrayProcessor 双路径、before.empty 静默丢弃等正确性问题；2D/3D 撤销核心算法对齐 | 开发组 |
