...
1. 导入位图图元
2. 通过 `insertEntityPreserveId()` 保留图元 ID
3. 在场景中查到实际图元
4. 通过 `findOrCreateLayerByType(Eg::LayerType::BITMAP)` 找到或创建位图图层
5. 调用 `assignEntityToLayer()` 分配图元
6. 设为当前图层
...
## 9. 图层动态刷新机制（2026-09-12）

### 9.1 问题背景

填充操作（Color Fill）会动态创建"填充图层"（`findOrCreateFillLayer`），但存在以下问题：

1. **右侧图层面板（RightToolBar）**：已正确连接 `QtLayerManagerBridge` 信号，能够自动刷新
2. **图层管理对话框（LayerManagerDialog）**：仅在构造函数时刷新图层列表，打开后不会响应动态创建的图层

### 9.2 修复方案

为 `LayerManagerDialog` 添加 `QtLayerManagerBridge` 信号连接：

```cpp
// LayerManagerDialog.h
explicit LayerManagerDialog(LayerEditService* layerEditService, QtLayerManagerBridge* bridge = nullptr, QWidget* parent = nullptr);
static void showDialog(LayerEditService* layerEditService, QWidget* parent, QtLayerManagerBridge* bridge = nullptr);
...
在 `findOrCreateFillLayer` 和 `writeRegionsToScene` 中添加调试日志，帮助定位图层分配问题：
