...
> **状态（2026-09-18）**：本文为背景分析/路线图（2026-08-05）。当前架构现状与后续计划以
> [`框架现状与修理计划.md`](框架现状与修理计划.md) 为准；本文保留逐项分析细节。
...
> 以下 P0 项已在后续迭代中全部解决，本文保留原始分析作为背景。

| P0 项 | 状态 | 说明 |
|---|---|---|
| 合并 `SceneDocument3D` 和 `SceneDocument3DAdapter` | ✅ 已解决 | 双文档模型已合并为单一文档入口 |
| 修复 `EntityToVertices` 缓存并发安全 | ✅ 已解决 | 缓存整块删除，`entityToVertices` 退回纯函数 |
| UICommon 对 Engine2D 依赖降级 | ✅ 已解决 | 改用内联前向声明，不再直接依赖 Engine2D，`Engine2DTypes.h` 已废弃 |
| 2D 离散化算法合并为 `Tessellator2D` | ✅ 已解决 | 全量与增量路径共用同一套离散化算法 |
| `RenderWidget3DAdapter` 空方法/恒真能力清理 | ✅ 已解决 | 空方法和恒真能力已移除或重写 |
| Engine3D 不再依赖 Engine2D | ✅ 已解决 | 依赖方向已纠正 |
| 2D/3D 文档模型明确 | ✅ 已解决 | 3D 双文档模型已合并 |
| `OperationRouting::setOperationBus()` 移除 | ✅ 已解决 | `dispatch()` 签名改为显式传递 `OperationBus*` |
| `DialogParameterBase` 导出宏修复 | ✅ 已解决 | 宏中添加 `UICOMMON_API` 确保 DLL 导出 |
