...
`RequiresUndo` / `RequiresRedo` 依赖的 `canUndo` / `canRedo` 不来自选择集 provider，而由 `setUndoRedoProvider` 单独提供，并且**覆盖**快照里的原值。该 provider 按值捕获 `UiServices::undoManager`，因此组合根必须填这个字段，否则两条规则永远不通过、Undo/Redo 永久置灰且快捷键一同失效（详见 `命令与状态流.md` 2.1 的接线约束）。
...
> 2D 的绘图工具定义已从同一份元数据视图生成：左侧绘图面板按命令目录顺序 + `CommandSurface2D::LeftToolbar`
> 过滤取 `CommandActionHub::toolAction()`，`createToolbars()` 不再拼装第二份工具列表（2026-08-26）。
---
...
当前菜单系统**只有配置驱动一条路径**，硬编码兜底已整条删除（2026-08-26）：
    ...
    → rebuildMenusFromConfig()
        ...
        → UiConfigurationManager::shared()                // 进程级唯一配置源
            ...
            └─ configData() == nullptr（连 san_yi.json 都加载失败）
                  → SY_ERRORF 后直接 return，菜单一栏都不建
    → bindConfiguredMenuState() → refreshConfiguredMenuState()
    → workbench->refreshCommandUiState()                  // 重推命令 UI 快照
```

> **为什么不再兜底**：`configData() == nullptr` 是装配级故障（资源没编进二进制、
> JSON 语法错误），必须显性报错，而不是悄悄换一套结构让人以为界面"只是有点怪"。
> 原来那条 `buildLegacyMenus()` 静态兜底与它牵出的整条硬编码菜单链
> （`buildFileMenu / buildViewMenu / buildHelpMenu / buildThemeMenu / addMenuAction`、
> 全部 `refresh*MenuForWorkbench`、`syncDrawMenuToTool`、`MenuState` 结构体与
> `m_menuState` 成员）已于 2026-08-26 删除，净减约 1310 行，
> `WorkbenchMenuManager.cpp` 从 1932 行降到 800 行。
> 判定依据：JSON 驱动路径 `rebuildMenusFromConfig()` 从不写 `m_menuState`，
> 而那些 refresh 函数一律以 `if (!m_menuState.xxx) return;` 开头 ——
> 在默认构建下 100% 空转，属于**看着有、其实从不执行**的死路径。
> 由此产生的 4 项功能缺口见 14.1。
...
> 注意 `File ▸ Recent Files` 目前只有静态的 `Open Recent` 条目，**动态文件列表未填充**（见 14.1）。
...
- 工具栏负责高频快捷入口
- 两者都必须引用同一个命令 ID
- 2D 的绘图工具定义直接复用命令目录元数据，不再各自维护一份列表

#### 5.4.1 Draw 菜单项与左侧工具栏按钮的关系（设计现状）

两者**共用派发链，不共用 QAction 实例**：

| | 配置驱动的 Draw 菜单项 | 左侧工具栏按钮 |
|---|---|---|
| QAction 来源 | `UiLayoutBuilder` 按 JSON 自建 | `CommandActionHub::toolAction()` 托管 |
| 是否可勾选 | 否（普通动作） | 是（`QActionGroup` 互斥） |
| 派发路径 | `commandId` → dispatcher → `OperationRouting` / `OperationBus` | 同一条 |
| 是否参与工具高亮 | 否 | 是 |

也就是说：从菜单点 `tool.line` 与从左侧栏点 `tool.line` 执行完全相同，但**高亮只出现在左侧工具栏**。
菜单项不显示当前激活工具，是设计现状，不是缺口 —— 要让菜单也跟着勾选，需要让 JSON 路径复用中枢实例，
会牵动 label / shortcut / checkable / workbench 过滤与 dispatcher 接线，当前不做。
...
3D 实际出现的顶层菜单：`File` / `Edit` / `View` / `Draw` / `Algorithm` / `Help`。

`Laser` / `Vision` 属于 2D 工作台，3D 下不出现——不是"少写了"，而是硬件与视觉模块
只在 2D 侧链接与接线（见 6.3 末尾说明）。
...
└─ Split
   ...
   └─ By Picked Plane      (model.split_by_pick_plane)   ← 命令保留，但视口表面拾取未实现，点击后弹提示并指向 Half by X/Y/Z


Algorithm
├─ Mesh Relief Engraving...  (algo.relief_engraving_mesh)
├─ Nesting from Mesh         (algo.nesting_from_mesh)
...
> **3D 下没有 Laser / Vision 顶层菜单，这是刻意的边界，不是遗漏。**
> `OperationSetup3D.cpp` 只注册 File / Edit / Algorithm / View / Help / Model 六族；
> `UI/3D/CMakeLists.txt` 不链接 Hardware、不链接 Vision。硬件与视觉归 2D 工作台。
> 历史上 `CommandCatalog3D` 曾注册 12 条 `laser.*` + 11 条 `vision.*` 且全部标 `Always`，
> 于是菜单项亮着、点了只有一条 `Operation not registered` 的 WARN —— 比灰掉更糟，已全部删除。
>
> `process.send_relief_to_laser` 同理移出 3D：3D 只负责「生成刀路 → 导出刀路」，
> 送激光由 2D 导入刀路后执行。原实现经 `LaserToolpathBridge` 转发，
> 而该桥的注册函数 `LaserToolpathBridgeRegistrar::registerFromService` 全仓零调用点，
> `isLaserToolpathAvailable()` 恒 false，点了必然失败。
>
> `algo.nesting_from_mesh` 保留在 3D：它是「3D 网格 → XY 投影件 → 排样」的桥接功能，
> Nesting 以 PRIVATE 链入 UI3D，实现完整可用。
>
> **命令 ID 不允许一名两义。** `algo.relief_engraving` 在 2D 映射
> `Algo_ReliefEngravingFromImage`（位图浮雕）、在 3D 映射 `Algo_ReliefEngraving`（网格浮雕），
> 曾以 `workbenches: ["2D","3D"]` 共用一条菜单项，两边干不同的事。
> 现拆为 `algo.relief_engraving`（2D）与 `algo.relief_engraving_mesh`（3D）。

> 已知空缺（命令目录尚未注册，JSON 里刻意不写，避免出现永久灰掉的入口）：
> `view.zoom_fit`（3D 已有语义等价的 `view.fit_all`，不重复注册同一行为）、
> `edit.group / ungroup`（3D 尚无「组」数据结构：`SceneManager3D` 是扁平容器，
> `SyGroup` / `GroupManager` 只在 Engine2D）、`file.export_pdf / export_png`（2D 专有）。
...
- 顶层分类骨架一致：共享 `base.json` 里同一份 `menus` 定义
  （File / Edit / View / Draw / Algorithm / Laser / Vision / Help），
  某个顶层菜单在某工作台是否出现，由其条目过滤后是否为空决定，不另写一套
...
- **一个 commandId 只有一种语义**：不允许同名命令在 2D / 3D 映射到不同操作
...
### 6.6 3D 视口鼠标交互约定（固定，不可配置）

3D 视口的鼠标键位是**编译期固定约定**，唯一事实来源是 `RenderWidget3D::mousePressEvent` / `mouseMoveEvent`：

- 中键拖动 = 旋转视图（orbit）
- 右键拖动 = 平移视图（pan）；右键**未拖动**时弹出右键菜单
- 滚轮 = 缩放
- 左键单击 = 拾取；`Ctrl+左键` = 切换选中，`Shift+左键` = 追加选中
- 左键拖动超过系统拖拽阈值 = 框选（橡皮筋），`Esc` 取消
- 左键双击 = 聚焦（命中实体聚焦该实体，空白处等价 `view.fit_all`）

约定要点：

1. **右键一键两用靠拖拽阈值区分**，不靠"右键不驱动相机"。`contextMenuEvent` 检查 `m_rightDragged`：拖过就只算平移、吞掉菜单事件。Windows 的 `WM_CONTEXTMENU` 晚于右键 release 到达，不做这个判定必然在松手处误弹菜单。
2. **与 2D 刻意不同**：2D 是中键 = 平移、右键不驱动相机（结构性分离）。3D 需要一个专用轨道键，中键给了旋转、平移落到右键。这条差异是有意的，不是漏改 —— 修改任一侧前先确认是否要连带改另一侧。
3. **不存在"导航方案/键位预设"配置**。历史上有过一套 `NavigationConfig3D` + `ViewportNavigationScheme`（11 个预设 + 自定义绑定表 + 设置页 UI + `3d/navigation/preset` 持久化），但它的 `loadDefaults()` 从来没有调用点，`RenderWidget3D` 也从不读 `m_settings.navigation` —— 是一整套改了不生效的假配置，已于 2026-08-26 连同设置页 UI、qrc 里的 `default_navigation_3d.json` 一并删除。后续不要重新引入"可配置键位"，除非同时把 `mousePressEvent` 改成真正读配置。
4. **状态栏提示必须与实现一致**：`RenderWidget3D::navigationStatusHint()` 返回上面这套键位的文字，`RenderWidget3DAdapter` 在 `sigCameraChanged` 时推给状态栏。此前它返回空串，等于每次动相机都把状态栏原有提示擦掉。
...
flowchart LR
    ...
    Switch -- "是" --> WB["主窗口直接处理<br/>triggerWorkbench / triggerTheme / LanguageManager / About"]
    Switch -- "否" --> HasWorkbench{"存在当前工作台<br/>m_workbench ?"}
    HasWorkbench -- "是" --> WB2["workbench->dispatchCommand(commandId)"]
    HasWorkbench -- "否" --> Warn["SY_WARNF 告警，命令不执行"]
    WB2 --> Catalog["2D: CommandCatalog / 3D: CommandCatalog3D<br/>按 commandId 解析到 OperationId"]
    Catalog --> Enrich["OperationRouting::enrichParams<br/>补旋转 angle / 对齐 mode / 单位 unit"]
    Enrich --> Run["OperationBus::run(OperationId, params, source)"]
```

```text
关键点
1. 所有菜单项（含工具栏、快捷键、右键）共用同一条命令分发链，保证行为一致。
2. 工作台切换 / 主题 / 语言 / About 是窗口级动作，在分发器处单独放行，不进入命令总线。
3. 未被命令目录识别的 commandId 不会执行，只有已注册命令可被触发。
4. 没有"无工作台时退回 OperationBus::run(opId)"的旁路。该分支已随 dispatchCommandSafely
   一起删除 —— 它绕过 enrichParams，会丢掉旋转 angle / 对齐 mode / 单位 unit。
```

### 8.3 启动自检：命令解析不出来的菜单项会被点名

配置里写了一个命令目录里不存在的 `commandId` 时，菜单项照样会被创建出来，只是永远
不会真正执行 —— 界面上看不出任何异常（不是灰的、也不报错），点了没反应。这类问题
历史上只能靠人工翻 JSON 发现。

`UiConfigSelfCheck`（`Main/Src/UI/ClientConfig/UiConfigSelfCheck.{h,cpp}`）在启动时
遍历配置里全部命令引用（**菜单 + 工具栏 + 右键菜单 + 快捷键**），逐条尝试在所属工作台的
命令目录里解析，把结果分五类记进日志。与菜单相关的三条判定规则：

1. **按声明的工作台判定**。`workbenches` 声明了 `2D` / `3D` 就只查对应目录；子项不写时
   继承上层菜单/子菜单的声明（JSON 里子项普遍不重复声明）。
2. **`workbenches` 为空表示「哪边都可能用」**，只要有一侧能解析就不算缺口。`shortcuts`
   节没有工作台字段，按同一条规则处理。
3. **窗口级命令跳过**。工作台切换 / 主题 / 语言这些命令刻意不进命令目录（由
   `MenuDispatcher` 短路，见 §8.2），判定直接复用
   `WorkbenchMenuManager::isWindowLevelCommand`，不在自检里另抄一份前缀名单 ——
   两份名单必然会漂移。

「目录里没有」还要再分一次因果：模块没编译导致的缺失（`BuildConfig::kGeoModelCore`
关闭时的 STEP / 布尔 / `model.*` 一批）记为 `gatedOutByBuild` 并只打 INFO，
真正的接线漏洞记为 `unresolvedCommands` 打 ERROR。不分开的话，每次裁剪构建都会
刷出一堆假告警，真问题反而被埋掉。

报告条目带定位路径（形如 `menus/edit/edit.transform/edit.move`），可以直接拿去翻 JSON。
自检**只报告、不阻止启动**。完整的五类报告、feature ↔ 构建开关对应表、以及配套的
构建期 qrc 护栏见 `UI定制.md` §10。

遍历逻辑与配置契约测试（`CommandUiWiringTests.cpp`）**共用同一份实现**：测试直接调
`UiConfigSelfCheck::collectCommandRefs`，不再自己抄一套。两边各写一份的话，会长出
「测试过了但运行期自检漏报」这种最难查的偏差。
...
用户点击菜单项
  ...
  → UiLayoutBuilder::bindAction 绑定的 lambda
      ...
      → MenuDispatcher::dispatch(commandId)           // 窗口级分发
          ├─ view.switch_to_2d / view.switch_to_3d    // 工作台切换，窗口级放行
          ├─ theme.* / language.* / help.about        // 同为窗口级，短路处理
...
> 这是**唯一**一条链：菜单、工具栏、右键菜单都经 `UiLayoutBuilder::bindAction` 接入
> 同一个 `MenuDispatcher`。`WorkbenchMenuManager::dispatchCommandSafely` 已随 legacy
> 菜单一起删除，不再存在第二条"直接 `OperationBus::run(opId)`"的旁路。
...
2. **配置驱动是唯一路径**：菜单结构由 JSON 配置驱动（base.json 为公共基线，客户配置 extends 继承）；配置取不到时报 `SY_ERRORF` 并不建菜单，没有硬编码回退。
...
- ✅ 硬编码 legacy 菜单链已整条删除，配置取不到时只报错不建菜单（见 4.1 与 14.1）

- ✅ 工具栏从 CommandCatalog 填充，与菜单/快捷键同源
- ✅ **左侧绘图工具栏已收口到命令中枢**（2026-08-26）：`DrawToolBarWidget` 重写为纯展示层，唯一 API 是 `setToolActions(QVector<QAction*>)`，按钮用 `QToolButton::setDefaultAction()` 挂 `CommandActionHub::toolAction()`，图标/文案/提示/可勾选/勾选态/启用态全由该 QAction 承载，点击即 trigger。删除了 `DrawToolEntry` / `setToolDefinitions` / `setOperationBus` / `updateActiveTool` / `currentActiveTool` 与自建按钮容器；`Workbench2D::buildDrawToolEntries()` 换成 `buildDrawToolActions()`（按目录顺序 + `CommandSurface2D::LeftToolbar` 过滤）。互斥交给中枢的 `QActionGroup`，不再用 `setAutoExclusive`。装配顺序有约束：面板必须在 `rebuildAllActions()` 之后创建，否则一个按钮都拿不到；启动日志 `[Workbench2D] Draw tool panel built: tools=18 host=ToolBar`
- ✅ 工具动作携带 `property("commandId")` 与同名 objectName（值取目录条目的 `shortcutId`，如 `tool.line`），与 `UiLayoutBuilder::bindAction` 写入的键一致；来源判定改读 `property("operationSource")`，不再靠宿主 objectName 猜（`setDefaultAction` 后关联对象是按钮本身，按名字匹配会误判成 `Shortcut`）
- ℹ️ 配置驱动的 Draw 菜单项（`tool.select` / `tool.line` 等）是 `UiLayoutBuilder` 自建的**不可勾选**普通 QAction，与中枢的工具 QAction 是两个对象：共用同一条派发链，但不参与勾选高亮 —— 高亮只在左侧工具栏。这是设计现状（见 5.4.1），不是缺口
...
- ✅ `ensureWorkbenchCompatibilityMenus()`（2D 下最小补齐 `Draw` / `View`）已随 legacy 链删除：JSON 配置本身就覆盖了这两个顶层菜单，补齐逻辑属于同一套死代码

- ⚠️ 3D 导出命令（file.export_obj / file.export_stl / file.export_step / file.export_pdf / file.export_png）尚未注册处理器，菜单项当前为禁用态（见 12.2 已知缺口）
- ✅ **菜单栏启用态已接入统一快照**（2026-08-25）：`WorkbenchMenuManager::refreshCommandStates(snapshot)` 按 `property("commandId")` → `CommandCatalog::menuIdForCommandId()` → `findByMenuId()` → `enableRule` → `Cmd::evaluateEnableRule()` 刷新，规则来源与顶部工具栏/右键菜单同一处。跳过 `commandUnavailable`（构建期永久禁用）与规则为 `Always` 的条目。`rebuildAllMenus()` 结尾回调 `m_workbench->refreshCommandUiState()`，保证重建后的新 QAction 立即拿到正确启用态
- ✅ **工具栏也纳入同一次遍历**（2026-08-26）：`forEachCommandAction()` 在遍历 `QMenuBar` 之后追加 `m_window->findChildren<QToolBar*>()`，并用 `QSet<QAction*>` 去重（中枢托管的 action 常同时挂在菜单和工具栏上）。此前只遍历菜单栏，表现为"菜单灰了工具栏还能点"；对已废弃 `m_menuState` 的那段 walk 同期删除
...
- ✅ **右键菜单分发器悬垂指针修复（3D 右键删除必崩）**（2026-08-27）：`UiLayoutBuilder::bindAction` 把 `IUiCommandDispatcher*` 裸指针捕进 QAction 的 `triggered` 闭包，闭包寿命跟随 QMenu；而 `Workbench2D/3D::buildConfiguredContextMenu` 在**栈上**建适配器再把地址传进去，函数一返回适配器即失效，菜单却是在调用方 `exec()` 的。点右键「删除」时 `dispatch()` 打在已被 `exec()` 调用链覆写的栈内存上 —— 日志停在 `[Workbench3D] Dispatch command='edit.delete'`（`m_services3D.operationBus` 读出垃圾非空值通过了判空），随后 `operationBus->run()` 访问违例，`[OperationBus3D] run` 这条 INFO 永远不出现。现让 `UiWorkbench` 直接实现 `IUiCommandDispatcher`（`dispatch()` 转发 `dispatchCommand()`），两处局部适配器删除，分发器与工作台同寿命。同时补上 3D 配置化右键菜单一直缺失的 `CommandActionHub3D::applySnapshotToMenu()`，与 2D 对齐
- ✅ 切换类命令（Group / Bezier）的 on/off 与启用语义抽成 `resolveToggleUiState()`，中枢托管动作与外部动作共用，菜单栏的 Group 项不再恒可点
- ✅ **快捷键收敛为 JSON 单一注册路径**（2026-08-26）：`bindShortcuts()` 不再自建 Undo/Redo 的窗口级 QAction。原来那两个用的是 `QKeySequence::Undo` / `Redo`，在 Windows 上正是 Ctrl+Z / Ctrl+Y，与 `configs/base.json` 里 Edit 菜单项自带的 `shortcut` 同键同范围，Qt 判 ambiguous 后两个接收者都不触发，表现为"按 Ctrl+Z 有时没反应"。启用态没丢：菜单 QAction 带 `commandId`，本就在 `forEachCommandAction()` 的遍历范围内
...
### 14.1 legacy 菜单删除后的已知缺口（待办）

2026-08-26 删除 legacy 菜单链时，以下 4 项入口随之消失。**共同背景：它们在默认构建下
本来也不执行** —— 全部挂在 `m_menuState` 上，而 JSON 驱动路径从不写这个成员，
相关 refresh 函数一律以 `if (!m_menuState.xxx) return;` 开头。所以这次是把
「隐性坏掉」变成「显性没有」，**不是这一轮改坏的**。后续要恢复，一律以 JSON 菜单项 +
命令目录条目的方式补，不要重新引入第二条构建路径。

- ⚠️ **View → TestView 入口**：`setTestViewHandler` 的注入点与 `TestViewWindow.h` 的
  include 一并删除。若仍需要该调试窗口，应注册成一条正常命令（如 `view.test_view`）
  并写进 JSON，而不是特设 handler。
- ⚠️ **File → Recent Files 的动态文件列表填充**：`populateRecentFilesMenu` 已删除。
  最近文件的**持久化读写仍在**，`file.open_recent` 命令也仍在命令目录里，缺的只是
  「把已记录的路径展开成子菜单项」这一步。恢复方式是给 JSON 菜单加动态段
  （参考右键菜单的 `dynamicSections` 机制），而不是在菜单管理器里手写 QAction。
- ⚠️ **Tools → Theme 顶层位置**：`buildThemeMenu` 已删除。**主题切换本身仍可用** ——
  `theme.*` 是窗口级命令，由 `MenuDispatcher` 短路到 `triggerTheme()`，入口在
  `Help → Theme`（JSON 定义）。缺的只是 legacy 那个额外的 `Tools` 顶层位置，
  而本文 4.1.1 明确约定不要 `Tools` 顶层菜单，因此这一项**倾向于不恢复**。
- ✅ **Modify 菜单的十项入口**（已结清，见 14.1.4）：`2d.move` / `2d.rotate` / `2d.scale` /
  `2d.copy` / `2d.mirror` / `2d.trim` / `2d.extend` / `2d.fillet` / `2d.chamfer` / `2d.delete`。
  注意这批用的是 `2d.*` 命名空间 —— 按 `menuIdForCommandId()` 的第 1 条规则，
  `2d.` 前缀直接返回 0 并回退 OperationId 分发，因此它们从来不参与启用态刷新。
  与 `Edit` 菜单里已有的 `edit.move / edit.scale / edit.mirror_*` 语义重叠，
  最终处置是「删 6 个重复别名 + 4 项改名为 edit.trim/extend/fillet/chamfer 并进 Edit 菜单」，
  不留两套命令 id（遵守 6.4 的"一名一义"）。

#### 14.1.1 复核结论（2026-08-27）

按现有代码把这 4 项重新盘了一遍，结论如下。**只有 Recent Files 是真缺口，
其余三项要么该关掉、要么被更底层的问题卡住。**

- **Tools → Theme：决定不恢复。** 4.1.1 已明确约定不要 `Tools` 顶层菜单，
  主题切换在 `Help → Theme` 完整可用。此项从待办中划掉，不再挂着。
- **View → TestView：决定不恢复，除非有人明确要这个调试窗口。**
  它是调试用途，恢复成本（一条 JSON 项 + 一条命令目录条目）虽低，
  但没有使用者。要恢复就按 `view.test_view` 走正常命令，别再开特设 handler。
- **Modify 十项：不是"补菜单"的活，是"先统一命名"的活。** 实测：
  - `edit.*` 已覆盖 6 项 —— `edit.delete` / `edit.copy` / `edit.move` /
    `edit.scale` / `edit.rotate_*` / `edit.mirror*`（`CommandCatalog.cpp:1178-1200`）。
    这 6 个 `2d.*` 是纯重复别名，应当**删除**，不要恢复。
  - 剩下 4 项 `2d.trim` / `2d.extend` / `2d.fillet` / `2d.chamfer`
    （`CommandCatalog.cpp:1223-1226`）是 `Edit_Trim/Extend/Fillet/Chamfer`
    这四个 OperationId 的**唯一**命令 id，没有 `edit.*` 同义词。
    正确做法是把它们**改名**为 `edit.trim/extend/fillet/chamfer`
    （`edit.` 前缀才参与 `menuIdForCommandId` 的启用态刷新），而不是并存两套。
  - **但先别急着加菜单项**：`CoreOperationRegistry.cpp` 里只注册了
    `Edit_Fillet`（:454）与 `Edit_Trim`（:1105）两个操作，
    `Edit_Extend` / `Edit_Chamfer` **根本没有实现**。
    现在把 4 项都写进 Edit 菜单，会有 2 项是点了没反应的死菜单。
    所以顺序是：先补 Extend/Chamfer 的操作实现（或明确只上 Trim/Fillet 两项），
    再改名，再加 JSON 菜单项。
- **File → Recent Files：真缺口，但卡在参数传递上，不是"填个子菜单"那么简单。**
  持久化与 `file.open_recent` 都在（`CommandCatalog.cpp:864-871`、
  `FileOperationRegistry.cpp:381-384`，后者读 `params["filePath"]`），
  但主菜单这条链**不能带参数**：
  - `IUiCommandDispatcher::dispatch(const QString& commandId)` 只有命令 id
    （`UiLayoutBuilder.h:29-39`）；
  - `Workbench2D::dispatchCommand` → `OperationRouting::dispatch(menuId)`
    用的是默认空 params（`UiWorkbench.cpp:311-329`），
    且 `enrichParams` 对 `File_OpenRecent` 不做任何填充、把传入 params 丢掉
    （`OperationRouting.cpp:108-140`）—— **3D 侧的 `OperationRouting3D.cpp:71-75`
    是透传的，这里两侧不一致，本身就该对齐**（违反 2D/3D 一致优先）。
  - 主菜单 JSON schema 里没有任何 "dynamic" 概念，`dynamicSections`
    只存在于右键菜单（`UiClientConfigBase.h:181`、`UiConfigLoader.cpp:679`、
    `UiContextMenuService.cpp:113-130`），而右键菜单之所以能"每次弹出都新鲜"，
    是因为它每次右键都整棵重建；全仓 `aboutToShow` 零命中。
  - 另外，最近文件当前有 **4 套并行读写**：`RecentFileService`（DB + QSettings
    `"RecentFiles"`）、`WorkbenchWindow::loadRecentFiles`（同逻辑复制一遍）、
    `ConfigManager::recentFiles`（另一个键 `"Files/RecentFiles"`）、
    `FileManager::recentFiles`（纯内存）。**动手前必须先收敛成一套**，
    否则菜单展开的是哪份数据都说不清。

  因此 Recent Files 的落地顺序是：①收敛 4 套最近文件读写为 `RecentFileService` 一套；
  ②给 `IUiCommandDispatcher` 补一个带 `QVariantMap` 的 dispatch 重载，
  并把 `OperationRouting::dispatch` 的 params 透传补上（与 3D 对齐）；
  ③主菜单 schema 加动态段（复用右键菜单的 `dynamicSections` 命名与注册机制，
  不要新造第二套概念）；④最后才是 JSON 里给 `file` 菜单挂上这一段。

#### 14.1.2 ①② 已完成（2026-08-27）

**① 最近文件收敛为一套。** 保留 `RecentFileService`（`Main/Src/UI/Services/RecentFileService.h`），
它现在实现 `IRecentFileService`（`UI/Common/Include/UI/Services/IRecentFileService.h`）。
删除的三套：`WorkbenchWindow` 里逐字节复制的 `addRecentFile/loadRecentFiles/saveRecentFiles`、
2D `ConfigManager` 的四个最近文件方法（写的是另一个键 `"Files/RecentFiles"`）、
2D `FileManager` 的纯内存 `recentFiles`。三套都零外部调用者。
契约同时砍掉了 `setPersistenceService/persistenceService`——它们要求的
`IPersistenceService` 全仓没有实现者，留着会让契约无法被实现。
注意 `IRecentFileService` **不加 `UICOMMON_API`**：纯虚接口没有跨 DLL 符号，
标成 dllimport 会让 MSVC 去 UICommon 里找并不存在的默认构造/析构，Main 侧实现类必然 LNK2019。

**② 参数透传统一到模板里。** `Cmd::dispatchFromCatalog`
（`UI/Common/Include/UI/Command/OperationRoutingBase.h`）新增 `incomingParams` 形参，
在调用 `enrich` 之前先播种：

```cpp
QVariantMap params = incomingParams;
enrich(menuId, op, params);
```

由此：2D 的 `OperationRouting::dispatch` 把自己的 `params` 传进模板（原先整个丢掉），
3D 的 `OperationRouting3D.cpp` 删掉 enrich 里的 `p = params;` 绕行写法，两侧同一条通路。
`enrich` 的语义定为"只补齐缺失项"，同名键写入即覆盖（菜单项自带语义优先，如 `edit.rotate_180` 的 `angle`）。

分发器一侧：`IUiCommandDispatcher::dispatch(commandId, params)` 是唯一纯虚入口，
无参 `dispatch(commandId)` 是基类里的非虚便捷转调，不构成第二个入口。
`UiWorkbench::dispatchCommand` / `Workbench2D` / `Workbench3D` 同步带上 params；
3D 侧改走完整 `OperationRequest3D`（便捷入口 `run(id, source)` 不带 params）。

回归：`UI/Common/Test/OperationRoutingBaseTests.cpp` 4 例
（参数到达 run、enrich 共存不覆盖、同名键以 enrich 为准、查不到映射不执行）。

#### 14.1.3 ③④ 已完成（2026-08-28）—— Recent Files 闭环

**③ 主菜单子菜单支持动态段，机制与右键菜单共用一套。**
`SubMenuDef` 新增 `dynamicSections`（`UiClientConfigBase.h`），JSON 键名与 `contextMenus` 完全相同；
`UiConfigLoader::parseSubMenu` 用同一行解析。段注册表仍是 `UiContextMenuService` 那一份，
新增 `fillDynamicSections(menu, sectionIds, ownerId)` 由两个表面共同调用：

- 右键菜单：`UiContextMenuService::buildMenu()`
- 主菜单子菜单：`UiLayoutBuilder::buildMenuItem()`

filler 类型随之从 `UiContextMenuSectionFiller` 改名 `UiMenuSectionFiller` —— 主菜单也在用，
名字不能再只提 ContextMenu。

**这是全仓第一处 `aboutToShow`，而且不可避免。** 主菜单只在工作台重建时构建一次，
最近文件列表随时在变；右键菜单不需要，因为每次右键都整块重建。实现上重填只丢弃
静态条目之后的部分（`staticCount` 之后），静态项的命令绑定与快捷键台账登记不受影响，
且非本菜单所有的 QAction 只摘不删（共享 QAction 不得进动态段）。

**踩到一个会让整件事静默失效的坎：** `WorkbenchMenuManager::filterMenusForWorkbench` 里
`filterSubMenu` 结尾是 `return !outSub.items.empty()`，而 Recent Files 是纯动态子菜单、
静态条目为空，会在 `UiLayoutBuilder` 看到它之前就被裁掉。已改为
`!outSub.items.empty() || !outSub.dynamicSections.isEmpty()`。

**④ 段提供者与 JSON 挂载。**
`WorkbenchMenuManager::registerRecentFilesSection()` 注册 `"file.recent"` 段：
读 `IRecentFileService::loadRecentFiles()`，只显示文件名（完整路径进 tooltip/statusTip，
否则 File 菜单会被长路径撑到半屏宽），点击时
`dispatch("file.open_recent", { filePath })` —— 这条链靠 ②的参数透传才通，
终点是 `FileOperationRegistry` 里早就注册好的 `ParamLambdaOperation`（读 `params["filePath"]`）。
`base.json` 的 `file.recent` 子菜单删掉那个死的静态占位项，改挂 `"dynamicSections": ["file.recent"]`。

三个刻意的决定：

- 落点选 `WorkbenchMenuManager` 而不是 `Workbench2D/3D`：最近文件是窗口级、2D/3D 共用，
  放进工作台就是两份闭包两处注销；本类与主窗口同寿，也就不需要像 `"layer.actions"` 那样在
  `deactivate()` 里注销。
- 菜单侧自己卡 10 条上限：服务的 QSettings 兜底路径截断了，但数据库路径
  （`RecentFileRepository::loadAll`）不截断。
- 动态条目**不设** `property("commandId")`：启用态刷新会按 commandId 去当前工作台目录反查规则，
  而 `file.open_recent` 只在 2D 目录里，3D 下会变成每次刷新告警一遍。这批条目恒可用
  （目录里 `File_OpenRecent` 的规则也是 `Always`），无需纳入联动。

回归：`ClientConfigTests.cpp` 新增 4 例 —— 解析、纯动态子菜单不被裁（含"既无静态项也无动态段
仍应被裁"的对照）、构建期先填一次 + `aboutToShow` 重填不叠加、静态项在重填后仍在原位且命令绑定完好。

至此 §14.1 的 4 个缺口只剩 Modify 十项（见 14.1.4），Theme 与 TestView 已判定不恢复。

#### 14.1.4 Modify 十项已结清（2026-08-30）

**先纠正 14.1.1 的一处过时判断：卡点已经不存在。** 那里写"`CoreOperationRegistry.cpp` 里只注册了
`Edit_Fillet`（:454）与 `Edit_Trim`（:1105），`Edit_Extend` / `Edit_Chamfer` 根本没有实现"。
按现在的代码复核，四个操作**全部已注册且是真实现**：

- `Edit_Fillet`（:467）→ `Eg::FilletChamfer::applyFillet`
- `Edit_Chamfer`（:491）→ `Eg::FilletChamfer::applyChamfer`
- `Edit_Trim`（:1118）→ 线段求交后改目标端点
- `Edit_Extend`（:1177）→ 同上，反向延长

**无参调用是可用路径。** `resolveTargetAndBoundary`（:246）在 params 里没有
`targetId` / `boundaryId` 时退化为"取当前选中的前两个图元"；圆角/倒角在 `radius` / `distance`
缺省（<0）时弹对话框取值。因此菜单点击（空 params）能走通，前提是选中两个图元 ——
这也决定了启用规则必须是 `RequiresUnlockedMultiSelection`：单选时若可点，操作会静默返回，
用户看到的就是"点了没反应"。

**六个重复别名删除，四个改名落到目录条目上。**
`2d.move / copy / rotate / scale / mirror / delete` 与 `edit.*` 完全同义，直接删；
`2d.trim / extend / fillet / chamfer` 不再作为别名存在，改为四条**目录条目**
（`CommandCatalog.cpp`，`shortcutId` = `edit.trim / edit.extend / edit.fillet / edit.chamfer`），
由 `operationIdForCommandId` / `menuIdForCommandId` 末尾的 shortcutId 循环统一解析 ——
不再往那两张 `map[]` / `precise[]` 里加第二处真值源。

删干净的理由不只是"一名一义"：`2d.` 前缀在 `menuIdForCommandId` 首行被短路成 0，
这批命令**永远不参与启用态刷新**，留着就等于留一条绕过启用规则的暗路。

**MenuActionId 补两个值，编号故意不连续。**
`Edit_Fillet = MENU_EDIT_BASE + 40`、`Edit_Chamfer = + 41`。不接在 `Edit_Extend`（+16）后面
是因为 `+17..+19` 已被 `Edit_Break` / `Edit_Join` / `Edit_Array` 占用，
而枚举值一旦发布就不能挪（会串台已保存的配置）。`Edit_Trim` / `Edit_Extend` 的"（预留）"
注释同时改成实际语义。

**JSON 挂在 `Edit ▸ Path Operations` 子菜单**（`base.json`）：Offset 之后加一组四项，
用分隔符与布尔运算隔开，`workbenches` 只有 `2D`。

**翻译只补两条。** `Trim` / `Extend` / `Fillet` / `Chamfer` 的中文在 `SanYiCAD_zh.ts` 里本来就有；
菜单 label 用了带省略号的 `Fillet...` / `Chamfer...`（会弹对话框，与 `Offset...` 一致），
这两个 source 是新的，按外科手术式补两条译文。其余语言文件连既有条目都是 `unfinished`，未动。

回归：`CommandUiWiringTests.cpp` 新增 2 例 —— 四项能解析出非零 MenuActionId + 目录条目规则是
`RequiresUnlockedMultiSelection` + 能解析出 OperationId；10 条旧 `2d.*` 别名确实已解析不到。
`ApplySnapshotToMenu_LeavesUnrelatedActionsUntouched` 里当哨兵用的 `2d.move` 换成仍在的
`2d.draw_line`。四个新 JSON 项由既有的 `CommandConfigContractTest` 与
`UiConfigSelfCheckTest.ShippedConfigsPassTheSelfCheck` 自动覆盖。

**遗留（未处理，需要拍板）：** `MenuActionId.h` 里 `Edit_PasteImage = MENU_EDIT_BASE + 10`
与 `Edit_Move = MENU_EDIT_BASE + 10` **值相同**，`findByMenuId` 在这两者之间是二义的。
改任一枚举值都可能影响已保存的配置，因此本轮没动。

至此 §14.1 的 4 个缺口全部结清：Recent Files 已闭环（14.1.2 / 14.1.3），
Modify 十项按"删 6 改 4"落地，Theme 与 TestView 判定不恢复。

### 14.2 语言 / 主题：落盘缺失与勾选态永空（2026-08-31 结清）

**现象。** 用户在 3D 菜单里选了中文，切到 2D 还是英文；重启后语言、主题都回到默认；
并且不管当前是什么语言 / 主题，`Help ▸ Language` 与 `Theme` 下的菜单项**一个勾都没有**，
用户无从知道当前生效的是哪一项。

**成因，四条独立的。**

1. **落盘缺失。** `MenuDispatcher::dispatch` 的 `language.*` 分支只调 `LM->setLanguage()`，
   改的是内存态；`theme.*` 分支同理。SQLite 的 `settings_common` 表（`common/language`、
   `common/theme`）从头到尾没人写。所谓"2D 切回来就没了"其实是 `SettingsService::init()`
   的 `applyCommonSettings()` 懒初始化**又把内存态按库里的旧值覆盖了一遍**。
2. **缺省值取 0。** `SettingsPersistenceHelper` 读不到键时用 `getInt(KEY_LANGUAGE, 0)`，
   即 English / Light。这把两种正常情形一起打死：首次运行（按系统语言解析出的结果被抹掉）、
   以及"菜单刚选完但还没落盘"。缺省值现在取**当前值**，读不到就不动。
3. **勾选态无人回填。** `refreshConfiguredMenuState()` 根本没有 `language.` / `theme.` 分支；
   `WorkbenchWindow::refreshThemeMenuChecks()` 函数体里没有任何一句 `setChecked`。
4. **重建后不补发。** `rebuildAllMenus()` 走的是 `menuBar()->clear()` 全量重建，产出的
   `QAction` 一律 `checked=false`（JSON 里不写 `checked`），而状态中心的信号只在状态
   **变化**时才发，重建后不会补发 —— 所以哪怕分支写对了，切一次工作台勾就丢了。

**为什么测试没抓住。** 现有 `CommandUiWiringTests` 只验"命令能不能解析出 MenuActionId"，
不验"点完之后有没有落库"，也不验勾选态；而落盘缺失在单次会话内是**看不出来的**
（内存态是对的），必须跨 `SettingsService::init()` 才暴露。

**修法。**

- `dispatch` 的 `language.*` 分支改为优先走 `SettingsService::setLanguage()`（服务已初始化时），
  否则退化到 `LM->setLanguage()`；主题分支同理走 `WorkbenchWindow::triggerTheme`。
  未知语言 code 直接 `SY_WARNF` + `return`，不再"解析失败就当英文"。
- 新增 `ThemeManager::themeFromMenuId()` / `menuIdFor()` 一对映射，菜单派发与勾选态回填
  共用同一份真值 —— 这是 §3.1「命令来源唯一」在状态回流方向上的对偶要求。
- `refreshConfiguredMenuState()` 补 `language.` / `theme.` 两个分支；
  `refreshThemeMenuChecks()` 退化为转调它，不再自己写一份。
- `rebuildAllMenus()` 末尾主动调一次 `refreshConfiguredMenuState()`，补上重建后的空档；
  该函数因此从 private 提到 public。
- 持久化位置收敛为一处：删掉 `AppInitializer.cpp` 里的 QSettings `General/Language`
  死键（从来只读不写），启动改走 `LanguageManager::resolveStartupLanguage()`，
  唯一权威是 SQLite `common/language`。
- `.qm` 缺失时 `LanguageManager::setLanguage` **保持当前语言并 WARN**，不再把
  `m_currentLanguage` 改成一个实际没加载的语言（那会让勾选态指向一个假状态）。
- `base.json` 删掉 `language.tw`：仓库里没有 `SanYiCAD_tw.qm`，它历史上被硬编码映射到简中，
  是一个"选了却不是它"的菜单项。要恢复须先补翻译资源。
- 删掉 `HelpLanguageMenuHelper` / `HelpThemeMenuHelper` 四个文件：配置化菜单接管后它们
  已无调用点，留着就是第二条构建路径（`UI/Common/README.md` 同步更新）。

**教训。** 配置驱动菜单的**状态回流**必须和构建走同一条路：`menuBar()->clear()` 之后所有
可勾选项都是空白态，任何"靠信号驱动勾选"的设计在重建点上都会漏。凡是全量重建，
重建完就得主动刷一次状态。
...
`WorkbenchMenuManager` 只以配置为准，不再保留兼容构建路径。

要求：

- 配置是唯一路径
- 配置取不到时报错并停止建菜单，不静默换结构
- 2D / 3D 切换一致
- 缺失的入口通过补 JSON 菜单项 + 命令目录条目恢复（见 14.1），不新增第二条构建路径
...
- 👉 构建路径唯一 (legacy 硬编码菜单链已删除，配置取不到即报错)
