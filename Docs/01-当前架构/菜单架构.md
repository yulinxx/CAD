...
├─ Import
│  ├─ All Supported...     (file.import_model / file.open)
│  ├─ 2D / Drawing Formats... (file.import_step)
│  ├─ 3D Model Formats...  (file.import_obj)
│  └─ Image Formats...     (file.import_image)
├─ Export
│  ├─ DXF                  (file.export_dxf)
│  ├─ SVG                  (file.export_svg)
│  ├─ PLT                  (file.export_plt)
...
flowchart TD
    ...
    Rebuild --> Resolve["确定 clientId<br/>（SANYI_CLIENT_ID 环境变量，缺省 san_yi）"]
...
- `Main/Src/UI/ClientConfig/configs/san_yi.json` — 三益标准版配置（默认运行时加载；不再覆盖 menus，仅覆盖工具栏/Dock/快捷键）
...
- ✅ File → Import / Export 采用文档的“聚合入口 + 格式项”结构，不再按格式碎片化
