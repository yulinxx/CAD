...
```text
SANYI_CLIENT_ID（编译期，缺省 san_yi）
  → UiConfigurationManager::applyConfiguration(":/configs/<clientId>.json")
      → UiConfigLoader::loadWithInheritance(path)
          → 解析 JSON
          → 若声明 extends → 递归加载父配置（如 san_yi → base）
          → mergeConfig：同 id 覆盖、新 id 追加、未声明字段继承父
  → UiConfigData（menus / toolBars / docks / shortcuts）
  → WorkbenchMenuManager::rebuildMenusFromConfig()
