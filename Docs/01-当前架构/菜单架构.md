...
- ⚠️ **Modify 菜单的十项入口**：`2d.move` / `2d.rotate` / `2d.scale` / `2d.copy` /
  `2d.mirror` / `2d.trim` / `2d.extend` / `2d.fillet` / `2d.chamfer` / `2d.delete`。
  ...
  恢复前先决定「保留哪一套命令 id」，不要两套并存（违反 6.4 的"一名一义"）。
...
至此 §14.1 的 4 个缺口只剩 Modify 十项（被 `Edit_Extend`/`Edit_Chamfer` 无注册操作卡住），
Theme 与 TestView 已判定不恢复。
