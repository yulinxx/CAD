...
### 3.4 变换类命令：零克隆状态交换（2026-09-24）

`EntitySnapshotsCommand` 不再 `clone()` before/after 两份状态，而是让**场景与命令各持
一份真实对象、撤销/重做时整批交换所有权**：

- 场景持 after 态时命令持 before 态，交换一次即互换；
- 「当前不在场景中的那一份」放在 `m_detached`，用 `m_beforeInScene` 双态标志记录它
  此刻是 before 还是 after（`SceneUndoCommands.h`）；
- 交换走 `SceneManager::replaceEntitiesById`，它**返回被换出的旧对象**，因此两侧永远
  各持一份、全程不 clone。撤销栈里不再存在任何深拷贝体。

**快照作用域 `SnapshotScope`**：状态交换默认只换「点名的 id」，但并非所有操作都只碰
点名图元 —— 偏移、布尔等算法会在场景内部自行增删图元：

| 取值 | 语义 | 使用者 |
|------|------|--------|
| `Subset`（默认） | 只交换点名 id 的状态 | 变换、属性面板编辑 |
| `WholeScene` | 额外把「目标状态之外」的场景图元摘下保管 | 偏移 / 布尔等算法包装（`AlgorithmTaskRegistration2D.cpp` 显式传入） |

漏掉 `WholeScene` 的症状：撤销时算法新生成的图元留在场景里、重做时被删掉的原件回不来，
反复撤销重做后图元数只增不减。`SceneEditService::pushExecutedChange` 的 `scope` 参数
就是为此存在的。

#### ⚠️ 连带约束：图元指针身份在撤销/重做后不成立

状态交换的语义是**换对象**，因此**跨一次撤销/重做缓存 `SyEntity*` 是不安全的** ——
缓存下来的指针会指向已被命令接管的旧对象。这条已经踩过三次：

- 选择集必须靠 `SceneManager` 内的 `replacePointers` 原地打补丁；
- `pGroup` / `pLayer` 必须从存活旧对象回填（代码注释：「快照克隆的 pGroup / pLayer
  可能已失效」）；
- 写性能基准时把 10 万个 `SyEntity*` 缓存起来、跨一次撤销复用，直接访问违例崩溃。

**需要跨撤销持有时请缓存 id 而不是指针**，每次用 `findSyEntityById` 重新解析
（该查找已是 O(1)）。
