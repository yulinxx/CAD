...
- 将 `UI/RenderCompat`、视口层、适配层逐步迁移到新 API
- 减少对旧专用接口的依赖
- 让渲染调用从 UI 语义中解耦
...
UI/RenderCompat/Src/RenderWidget.cpp      // 逐步迁移到新 API
```
...
#### 7. `UI/RenderCompat/Src/RenderWidget.cpp`
**修改目标**
...
#### 8. `UI/RenderCompat/Src/SceneGeometrySinkAdapter.cpp`
**修改目标**
...
- [x] `SanYiRender.dll` 编译通过，无新增编译错误
- [x] `UICommon`、`UI3D`、`UI2D`、`RenderCompat` 均编译通过
...
继续减少 UI/RenderCompat 对旧 `renderSet*` / `renderEmit*` wrapper 的直接依赖。
...
文件：`UI/RenderCompat/Src/SceneGeometrySinkAdapter.cpp`
...
文件：`UI/RenderCompat/Src/RenderWidget.cpp`
...
- [x] 编译验证通过：SanYiRender.dll、UI2D、UI3D、UICommon、RenderCompat
