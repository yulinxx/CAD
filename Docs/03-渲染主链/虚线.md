...
> 历史背景：早期实现把虚线按“每一条折线边”切段（整条边要么全实、要么全空），导致多边形选中后整条边消失、图元“变了样”。现方案改为**按沿路径累积弧长**施加虚线，从根本上解决了这个问题。
...
### 9.3 旧 ConfigManager（QSettings，遗留）

早期遗留键（`UI/2D/Src/Manager/ConfigManager/ConfigManager.{h,cpp}`），当前渲染链路已不使用，仅保留兼容：

| QSettings 键 | 默认值 |
|--------------|--------|
| `View/SelectionDashLength` | `10.0` |
| `View/SelectionDashGap` | `8.0` |
| `View/SelectionDashSpeed` | `48.0` |
