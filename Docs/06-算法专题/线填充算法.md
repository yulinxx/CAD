...
菜单/工具栏 (commandId)
  ...
  ▼ OperationBus → CoreOperationRegistry（委托给子注册器）
...
```mermaid
flowchart TD
    A["输入：N 个轮廓的边集"] --> B["对每个图元边集算 BBox"]
    B --> C["bboxesOverlap(a, b) — 真实重叠检测"]
    C --> D["两个方向交集 > kNestBBoxMargin(1.0)"]
    D --> E["用并查集求连通分量"]
    E --> F["重叠的轮廓归入同一组"]
    
    F --> G{"三种情形"}
    G -- "外轮廓 + 孔洞" --> H["BBox 包含 → 同组 → 孔洞奇偶为偶 → 不填充"]
    G -- "两个相交轮廓" --> I["同组 → 相交部分奇偶为偶 → 挖空"]
    G -- "完全分离轮廓" --> J["不同组 → 互不干扰"]
```
...
```mermaid
flowchart TD
    A["computeScanRange(minY, maxY, nStep, direction)"] --> A1["B2T: startY=minY, endY=maxY, stepDir=+nStep"]
    A --> A2["T2B: startY=maxY, endY=minY, stepDir=-nStep"]
    A1 --> A3["起点对齐到 nStep 网格"]
    A2 --> A3
    
    A3 --> B{"nY 在方向内 ?"}
    B -- 是 --> C["确定扫描方向 bCurL2R"]
    C --> D{"Bi 模式 ?"}
    D -- 是 --> D1["逐行交替：nScanIdx % 2 == 0"]
    D -- 否 --> D2["fillType == L2R"]
    D1 --> E["遍历每个填充组"]
    D2 --> E
    E --> F["gatherGroupEdges(group) — 收集本组边"]
    F --> G["computeFillSegmentsForEdges(nY, edges, group)"]
    G --> H["求交 + 配对 + 内外判定"]
    H --> I["遍历每个填充段"]
    I --> J["x /= nPrecision — 还原实数域"]
    J --> K{"bCurL2R ?"}
    K -- 否 --> K1["交换起终点 — 决定激光走向"]
    K -- 是 --> L["rotatePoint(pt, +dAngle) — 反旋转回原角度"]
    K1 --> L
    L --> M["insertPts(ptS); insertPts(ptE); insertFillLine()"]
    M --> I
    I -- 完成 --> B
    B -- 否 --> N["输出填充线集合"]
```
...
```mermaid
flowchart TD
    A["采样 Y = nY + 0.5（偏移避开退化）"] --> B["遍历每条边"]
    B --> C{"边类型 ?"}
    C -- "普通/垂直边" --> D["edgeCrossesScanY — 半开区间 (yMin, yMax]"]
    D --> D1["交点 X 由 dDelta 线性求出"]
    C -- "水平边" --> E{"本行 Y 与边重合 ?"}
    E -- 是 --> E1["两个端点 X 都加入交点表"]
    E -- 否 --> F["跳过"]
    C -- "端点贴行" --> G["端点恰好落在整数行 dRowY 上"]
    G --> H["额外补一个交点，避免漏配"]
    
    D1 --> I["sortAndDedupeIntersections — 按 X 升序排序"]
    E1 --> I
    H --> I
    I --> I1["只去重"同一条边 + 同一 X""]
    I1 --> I2["不同边的重合 X 必须保留"]
...
flowchart TD
    A["输入：排序后的交点列表"] --> B["i = 0"]
    B --> C{"i < 交点数 ?"}
    C -- 是 --> D["先试相邻配对 (x[i], x[i+1])"]
    D --> E["isSegmentInsideGroup 校验"]
    E --> F{"校验通过 ?"}
    F -- 是 --> G["收下该配对，i += 2"]
    G --> C
    F -- 否 --> H["回退搜索：在 j > i 中找跨度最短且校验通过的 x[j]"]
    H --> I{"找到 ?"}
    I -- 是 --> J["配对 (x[i], x[j])，i = j + 1"]
    J --> C
    I -- 否 --> K["跳过，避免死循环"]
    K --> C
    C -- 否 --> L["输出配对后的填充段"]
