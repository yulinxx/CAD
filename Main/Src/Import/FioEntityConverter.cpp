...
std::unique_ptr<Eg::SyEntity> FioEntityConverter::convertEntity(const Fio::EntityInfo& info, const Fio::BinaryBlob& blob)
{
    switch (info.type)
    {
    ...
    case Fio::EntityType::Spline:
    {
        ...
        {
            const auto* raw = reinterpret_cast<const double*>(blob.data + info.extensionDataOffset);
            uint32_t offset = 0;
    ...
    case Fio::EntityType::Image:
    {
        ...
        if (info.extensionDataSize > 0)
        {
            // 图像像素数据从扩展数据块中读取
            e->setPixelData(
                reinterpret_cast<const unsigned char*>(blob.data + info.extensionDataOffset), info.extensionDataSize);
        }
    ...
    case Fio::EntityType::Mesh3D:
    {
        ...
        {
            const float* raw = reinterpret_cast<const float*>(blob.data + info.extensionDataOffset);
            ...
            {
                e->normals.emplace_back(
                    raw[normalOffset + i * 3], raw[normalOffset + i * 3 + 1], raw[normalOffset + i * 3 + 2]);
            }
