...
std::vector<RenderBatch> SceneTraverser::traverse2D(Eg::SceneManager* scene, const RenderContext& context)
{
    ...
    SY_DEBUGF("[SceneTraverser] traverse2D processing %d entities, %d selected",
    ...
    SY_INFOF("[SceneTraverser] traverse2D completed batches=%d", static_cast<int>(batches.size()));
    return batches;
}
...
std::vector<RenderBatch> SceneTraverser::traverse3D(SceneDocument3D* document, const RenderContext& context)
{
    ...
    SY_DEBUGF("[SceneTraverser] traverse3D processing %d nodes, %d selected",
    ...
    SY_INFOF("[SceneTraverser] traverse3D completed batches=%d", static_cast<int>(batches.size()));
    return batches;
}