...
UiLayoutBuilder::UiLayoutBuilder(QMainWindow* window,
    ...
    const QString& workbenchId)
{
    ...
    else
    {
        ...
        action->setToolTip(QCoreApplication::translate("UiLayoutBuilder",
            "Feature not enabled (command: %1)").arg(commandId));
...
void UiLayoutBuilder::buildMenuItem(QMenu* parent,
...
void UiLayoutBuilder::buildDocks(const std::vector<DockDef>& docks)
{
    ...
    for (const auto& dock : docks)
    {
        ...
        QWidget* panel = m_panelRegistry
            ? m_panelRegistry->createPanel(dock.widgetType, m_window)
            : nullptr;
        if (!panel)
        {
            // 面板工厂未注册 → 占位提示，优雅降级
            panel = new QWidget(m_window);
            SY_WARNF("[UiLayoutBuilder] Panel '%s' not registered, using placeholder",
