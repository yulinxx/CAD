...
namespace
{
    ...
    ::testing::Environment* const g_testEnv =
        ::testing::AddGlobalTestEnvironment((new TestAppEnvironment));
...
{
    const QString path = writeTempConfig(QStringLiteral("minimal.json"),
        QStringLiteral(R"({
...
TEST(ClientConfigLoaderTest, ExtendsMergesParentConfig)
{
    ...
    const QString basePath = writeTempConfig(QStringLiteral("base_merge.json"),
        QStringLiteral(R"({
            "meta": { "clientId": "base" },
            "menus": [
    ...
    const QString childPath = writeTempConfig(QStringLiteral("child_merge.json"),
        QStringLiteral(R"({
            "meta": { "clientId": "child" },
            "extends": "<BASE>",
            "menus": [
...
TEST(ClientConfigPanelRegistryTest, RegisterCreateQuery)
{
    ...
    registry.registerPanel(QStringLiteral("PanelA"),
...
{
    UiPanelRegistry registry;
    registry.registerPanel(QStringLiteral("P"),
        [](QWidget* parent) -> QWidget* { return new QWidget(parent); });
    registry.registerPanel(QStringLiteral("P"),
        [](QWidget* parent) -> QWidget* { return new QMainWindow(parent); });
...
{
    UiPanelRegistry registry;
    registry.registerPanel(QStringLiteral("PanelX"),
        [](QWidget* parent) { return new QWidget(parent); });
...
{
    const QString path = writeTempConfig(QStringLiteral("mgr_test.json"),
        QStringLiteral(R"({
            "meta": { "clientId": "mgr" },
...
TEST(ClientConfigManagerTest, StrictPolicyFailsOnMissingConfig)
{
    ...
    EXPECT_FALSE(mgr.applyConfiguration(QStringLiteral(":/configs/nope.json"),
        ConfigFallbackPolicy::Strict));
    EXPECT_EQ(mgr.configData(), nullptr);
}

TEST(ClientConfigManagerTest, ResetClearsState)
{
    const QString path = writeTempConfig(QStringLiteral("mgr_reset.json"),
        QStringLiteral(R"({ "meta": { "clientId": "mgr" } })"));
...
{
    const QString path = writeTempConfig(QStringLiteral("workbench_filter.json"),
        QStringLiteral(R"({
            "meta": { "clientId": "wb" },
            "menus": [
...
{
    auto commandAvailable = [](const QString& commandId) {
        return commandId == QStringLiteral("file.open")
            || commandId == QStringLiteral("view.grid")
            || commandId == QStringLiteral("view.wireframe")
            || commandId == QStringLiteral("model.make_box")
    ...
    const auto filtered2D = WorkbenchMenuManager::filterMenusForWorkbench(
        configMenus, QStringLiteral("2D"), commandAvailable, QStringLiteral("2D"));
    const auto filtered3D = WorkbenchMenuManager::filterMenusForWorkbench(
        configMenus, QStringLiteral("3D"), commandAvailable, QStringLiteral("3D"));
    const auto filtered2DAgain = WorkbenchMenuManager::filterMenusForWorkbench(
        configMenus, QStringLiteral("2D"), commandAvailable, QStringLiteral("2D"));
