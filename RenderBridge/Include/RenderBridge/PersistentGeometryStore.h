 ...
 * ## 类型隔离说明
 *
 * 本类只面向渲染抽象层：几何仓与绘制列表都通过 `IRenderDevice` /
 * `IRenderScene` 操作，句柄是 RenderAbstraction 的 POD 包装。本文件头与
 * 实现都不 include renderx.h —— 后端的知识全在适配层里。
 */
...
{
    /// 抽象层的块与统计原样使用，不再本地复刻一份（见 IRenderTypes.h）
    using GeometryBlock = RenderAbstraction::GeometryBlock;
    using GeometryStoreStats = RenderAbstraction::GeometryStoreStats;
    ...
    public:
        ...
        struct Config
        {
            ...
            /// 分配粒度（字节），0 表示用后端默认（256，已按 vec4 对齐）
            ...
            /// 条目包围盒与提交范围用哪套剔除判据（2D 世界矩形 / 3D 视锥）。
            /// 一次定死：列表内所有条目必须同属一套判据
        ...
        /**
         * @brief 在给定设备上创建几何仓，在给定场景上创建绘制列表
         *
         * 重复调用会先释放旧的。任一步失败都会回收已建好的部分并返回 false，
         * 不留半个资源。
         *
         * @param scene 绘制列表的宿主。**可以为空**：几何仓的分配/写入/释放
         *              只需要设备，契约测试用不上绘制列表，此时 drawList() 返回
         *              无效句柄，槽位操作退化为空操作。
         */
        ...
        /// 绘制列表句柄。initialize 未给 scene 时无效
        RenderAbstraction::DrawListHandle drawList() const { return m_drawList; }
        /// 块所在仓的句柄。分片之后块可能落在任一仓，没有「当前仓」这种东西
        RenderAbstraction::GeometryStoreHandle storeForBlock(uint64_t blockId) const;
        ...
        /**
         * @brief 写入 / 更新一个槽位上的绘制命令
         *
         * 槽号由 acquireSlot 分配；重写同一个槽号即更新，不新增条目。
         * @param bounds 世界空间包围盒，按 Config::drawListViewType 解读；
         *               传 nullptr 表示该条目永不剔除
         */
        bool upsertDrawItem(uint32_t slot, const RenderAbstraction::DrawInstruction& command,
                            const RenderAbstraction::Aabb3* bounds);
         ...
         * 让它失效（后端原地改写句柄槽位）。因此调用方填
         * `DrawInstruction::vertexBuffer` 时直接用 `out.buffer` 即可，不要再去取
         * "当前缓冲"（多仓下没有这种东西）。
         */
    ...
    private:
        ...
        /// 设备与场景都不持有；nullptr 表示未初始化
        RenderAbstraction::IRenderDevice* m_device = nullptr;
        /// 绘制列表的宿主，可为空（见 initialize）
        RenderAbstraction::IRenderScene* m_scene = nullptr;

        std::vector<RenderAbstraction::GeometryStoreHandle> m_stores;  // 下标 0 是主仓
        ...
        RenderAbstraction::DrawListHandle m_drawList;
